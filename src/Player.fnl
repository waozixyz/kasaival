...
{:scale 1 :element "fire" :ow 31 :oh 175
 ...
 :init (fn init [self t]
         ...
         (set self.hp (or t.hp 1500))
         (set self.lvl (or t.lvl 0))
         (set self.speed (or t.speed 400))
 ...
 :draw (fn draw [self]
         (gr.setColor 1 1 1)
         (sprite:draw self.x self.y 0 self.scale self.scale (* self.ow .5) self.oh))
 ...
 :update (fn update [self dt g]
           (if (> self.hp 2900)
             ...
             (set self.hp (- self.hp 6))
             (> self.hp 800)
             (set self.hp (- self.hp 9))
             (> self.hp 700)
             (set self.hp (- self.hp 17))
             (set self.hp (- self.hp 30)))
           (sprite:update dt))}

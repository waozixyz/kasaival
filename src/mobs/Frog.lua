...
local function init(self,spawn)
   self.move = 0 
    ...
    self.anime = Animation:init(gfx.newImage("assets/mobs/frog.png"), 64, 64, 1)
    return copy(self)
end
...
local function draw(self)
    local sx, sy = 1, 1
    gfx.setColor(1,1,1)
    gfx.draw(self.anime.spriteSheet, self.anime.quads[self.anime:spritenumber(1,8)],self.x+self.move*self.direction, self.y+math.sin(self.anime.currentTime-math.pi/8)^2*20, 0, self.direction*-1, 1, sx, sy)
end
